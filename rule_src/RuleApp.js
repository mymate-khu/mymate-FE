...
const RuleApp = () => {
  ...
  setIsLoading(true);
  try {
    const jsonResponse = await makeApiCall("/api/rulebooks", "GET");
  ...
  try {
    await makeApiCall("/api/rulebooks", "POST", {
      title: newTask,
  ...
  try {
    await makeApiCall(`/api/rulebooks/${id}`, "DELETE");
  ...
  try {
    await makeApiCall(`/api/rulebooks/${item.rulebookId}`, "PUT", {
      title: item.text,
