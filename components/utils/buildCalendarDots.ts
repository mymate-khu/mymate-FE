...
/** 서버 배열 -> Calendar markedDates 포맷으로 변환 (내 항목 우선 정렬) */
export function buildCalendarDots(
  ...
  for (const date of Object.keys(byDate)) {
    const arr = byDate[date].slice().sort((a, b) => {
      const aMine = String(a.memberId) === String(myid);
      const bMine = String(b.memberId) === String(myid);
    ...
    const dots = arr.map((it) => {
      const isMine = String(it.memberId) === String(myid);
      return {
        key: `${it.id}-${date}`, // 날짜와 id를 섞어 유니크 보장 (임의키 OK)
        color: isMine ? MINE_COLOR : OTHERS_COLOR,
